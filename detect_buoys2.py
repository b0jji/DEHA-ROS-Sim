...
def find_color(frame, points, max_contour_area, prev_contours):
    ...
    for i, c in enumerate(cnts):
        ...
        if area > 3000 and area < max_contour_area:
...
def draw_midway_line(frame, yellow_contour_info, red_contours, green_contours):
    if yellow_contour_info:  # If yellow contour is detected
        ...
        if furthest_contour:
            ...
            # Publish the thrust angle and command
            publish_thrust_angle_and_cmd(x_mapped)
...
def publish_thrust_angle_and_cmd(x_mapped):
    global pub_thrust_angle, pub_thrust_cmd
    # Map the x_mapped value from the range [-180, 180] to [-1.57, 1.57]
    thrust_angle = (x_mapped / 180) * 1.40
    # Publish the thrust angle
    pub_thrust_angle.publish(thrust_angle)
    # Map the x_mapped value from the range [-180, 180] to [1, 0]
    thrust_cmd = 1 - abs(x_mapped) / 180
    # Publish the thrust command
    pub_thrust_cmd.publish(thrust_cmd)
...
def listener():
    ...
    # Create publishers for the thrust angle and command
    pub_thrust_angle = rospy.Publisher('/wamv/thrusters/middle_thrust_angle', Float32, queue_size=10)
    pub_thrust_cmd = rospy.Publisher('/wamv/thrusters/middle_thrust_cmd', Float32, queue_size=10)
